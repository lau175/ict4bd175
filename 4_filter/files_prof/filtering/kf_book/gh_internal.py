...
def plot_hypothesis2():
    with book_plots.figsize(y=2.5):
        plt.figure()
        plt.errorbar(range(1, 11), [69, 70, 69,71, 70, 71, 69, 70, 69, 70],
                     xerr=0, yerr=6, fmt='bo', capthick=2, capsize=10)
        plt.plot([1, 10], [69, 70.5], color='g', ls='--')
...
def plot_estimate_chart_1():
    with figsize(y=2.5):
        plt.figure()
        ax = plt.axes()
        ax.annotate('', xy=[1,59], xytext=[0,58],
                    arrowprops=dict(arrowstyle='->', ec='r',shrinkA=6, lw=3,shrinkB=5))
        plt.scatter ([0], [58], c='b')
        plt.scatter ([1], [59], c='r')
        plt.xlabel('day')
        plt.ylabel('weight (kg)')
...
def plot_estimate_chart_2():
    with figsize(y=2.5):
        ...
        plt.scatter ([0], [58.0], c='k',s=128)
        plt.scatter ([1], [64.2], c='b',s=128)
        plt.scatter ([1], [59], c='r', s=128)
        plt.text (1.0, 58.8, "prediction ($x_t)$", ha='center',va='top',fontsize=18,color='red')
        plt.text (1.0, 64.4, "measurement ($z_t$)",ha='center',va='bottom',fontsize=18,color='blue')
        plt.text (0.0, 59.8, "last estimate ($\hat{x}_{t-1}$)", ha='left', va='top',fontsize=18)
        plt.xlabel('day')
        plt.ylabel('weight (kg)')
        ax.xaxis.grid(True, which="major", linestyle='dotted')
        ax.yaxis.grid(True, which="major", linestyle='dotted')
...
def plot_estimate_chart_3():
    with figsize(y=2.5):
        ...
        ax.annotate('', xy=[1,59], xytext=[1,64.2],
                    arrowprops=dict(arrowstyle='-',
        ...
        plt.scatter ([1], [59], c='r', s=128)
        plt.text (1.0, 58.8, "prediction ($x_t)$", ha='center',va='top',fontsize=18,color='red')
        plt.text (1.0, 64.4, "measurement ($z$)",ha='center',va='bottom',fontsize=18,color='blue')
        plt.text (0, 59.8, "last estimate ($\hat{x}_{t-1}$)", ha='left', va='top',fontsize=18)
