...
class MySubscriber:
		...
		def myOnMessageReceived (self, paho_mqtt , userdata, msg):
			...
			global k
			k+=1
			print ("Topic:'" + msg.topic+"', QoS: '"+str(msg.qos)+"' Message: '"+str(msg.payload) + "'"+ "counter: ", k)
			#k=0
			try:
				#k+=1
				...
				json_body = [
					...
					#"time": datetime.datetime.fromtimestamp(data['time_stamp']).strftime("%Y-%m-%d %H:%M:%S"),
					"time": int(data['time_stamp']),
					"fields":
						{
						"value": data['value']
						}
